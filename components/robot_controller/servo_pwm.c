...
esp_err_t servo_pwm_init(servo_pwm_t *servo, const servo_pwm_cfg_t *cfg)
{
    ...
    if (cfg->freq_hz == 0 || cfg->min_pulse_us >= cfg->max_pulse_us) return ESP_ERR_INVALID_ARG;
    ...
    // Zero duty emits no control pulse, so initialization cannot command motion.
    servo->initialized = true;
    ESP_LOGI(TAG, "servo pwm gpio=%d freq=%" PRIu32 "Hz; output disabled",
             (int)servo->gpio, cfg->freq_hz);
    return ESP_OK;
}

esp_err_t servo_pwm_enable(servo_pwm_t *servo)
{
    if (!servo) return ESP_ERR_INVALID_ARG;
    if (!servo->initialized) return ESP_ERR_INVALID_STATE;

    // Enabling only grants subsequent angle commands; it does not emit a pulse.
    servo->enabled = true;
    return ESP_OK;
}

esp_err_t servo_pwm_disable(servo_pwm_t *servo)
{
    if (!servo) return ESP_ERR_INVALID_ARG;
    if (!servo->initialized) return ESP_ERR_INVALID_STATE;

    // Stop pulses before publishing the disabled state.
    ESP_RETURN_ON_ERROR(ledc_set_duty(LEDC_LOW_SPEED_MODE, servo->ledc_channel, 0),
                        TAG, "disable duty");
    ESP_RETURN_ON_ERROR(ledc_update_duty(LEDC_LOW_SPEED_MODE, servo->ledc_channel),
                        TAG, "disable update");
    servo->enabled = false;
    return ESP_OK;
}
...
{
    if (!servo || !cfg) return ESP_ERR_INVALID_ARG;
    if (!servo->initialized || !servo->enabled) return ESP_ERR_INVALID_STATE;
    if (cfg->freq_hz == 0 || cfg->min_pulse_us >= cfg->max_pulse_us) return ESP_ERR_INVALID_ARG;
    if (!isfinite(angle_deg)) return ESP_ERR_INVALID_ARG;
