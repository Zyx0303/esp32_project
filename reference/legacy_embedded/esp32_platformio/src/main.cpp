...
const char* password = ROBOT_WIFI_PASSWORD;
const char* serverIP = ROBOT_SERVER_IP;
const uint16_t port = ROBOT_TCP_PORT;
            ...
            if (available > 0) {
                ...
                if (cmdIndex == sizeof(ControlCommand)) {
                    ...
                    if (cmd.head1 == FRAME_HEAD1 &&
                        ...
                        // 发送命令到STM32
                        Serial1.write(cmdBuffer, sizeof(ControlCommand));
                        Serial1.flush();
...
void handleSerial2Task(void* parameter) {
    ...
    while (true) {
        ...
        if (Serial1.available()) {
            size_t bytesToRead = min(Serial1.available(), static_cast<int>(sizeof(tcpBuffer)));
            if (bytesToRead > 0) {
                size_t bytesRead = Serial1.readBytes(reinterpret_cast<char*>(tcpBuffer), bytesToRead);
                if (bytesRead > 0) {
...
void handleTCPTask(void* parameter) {
    ...
    while (true) {
        if (!client.connected()) {
            ...
            if (!wifiReady()) {
#if ROBOT_WIFI_AP_MODE
                Serial.println("本地接入点未就绪，重启ESP32...");
                ESP.restart();
#else
...
void setup() {
    ...
    Serial1.begin(BAUD_RATE, SERIAL_8N1, 16, 17);
    ...
    if (!WiFi.softAP(ssid, password, 6, false, 4)) {
        Serial.println("创建机器人本地接入点失败");
        ESP.restart();
    }
    Serial.println("机器人本地接入点已启动");
    Serial.printf("ESP32 AP地址: %s\n", WiFi.softAPIP().toString().c_str());
#else
