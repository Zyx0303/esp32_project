...
class CPGNetwork:
    ...
    def __init__(self, params: CPGParams, use_rotation_matrix: bool = True):
        ...
        for i in range(self.n_oscillators):
            ...
            # 添加环形耦合（首尾相连）
            if i == 0:
                self.coupling_matrix[i][self.n_oscillators-1] = lambda_ * 0.5
            if i == self.n_oscillators - 1:
                self.coupling_matrix[i][0] = lambda_ * 0.5
    ...
    def update(self, dt: float) -> np.ndarray:
        ...
        for i, osc in enumerate(self.oscillators):
            ...
            # 改进的转向效果计算
            radius_factor = 1.0 - np.exp(-i / (self.n_oscillators/3))
            turning_effect = self.params.turning_bias * radius_factor

            # 添加渐进式补偿
            angular_position = 2 * np.pi * i / self.n_oscillators
            offset = self.params.turning_offset * np.sin(angular_position)
...
class SnakeRobot:
    ...
    def calculate_positions(self, joint_angles: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # 更新头部方向角
        # 根据转向偏置调整转向角速度
        turning_rate = np.deg2rad(self.params.turning_bias) * self.params.dt
        self.theta += turning_rate  # 累积转向角度
        ...
        for i in range(self.params.n_segments):
            if i == 0:
                ...
                # 累积角度（考虑整体转向）
                total_angle = self.theta + np.deg2rad(relative_angle)
                # 计算节段位置
                x_pos[i] = x_pos[i-1] - self.params.segment_length * np.cos(total_angle)
                y_pos[i] = y_pos[i-1] - self.params.segment_length * np.sin(total_angle)
...
def create_circular_motion_params():
    """创建圆周运动参数"""
    return CPGParams(
        n_segments=8,           # 增加节段数以获得更好的圆周效果
        segment_length=0.2,
        frequency=1.0,          # 降低频率使运动更稳定
        amplitude=30.0,         # 适当的振幅
        phase_bias=np.pi/3,     # 使用90度相位差
        coupling_weight=1.2,    # 适当的耦合强度
        convergence_rate=1.0,   # 标准收敛率
        mu_t=0.3,
        mu_n=1.0,
        control_gain=15.0,
        v_f=0.2,               # 降低速度使圆周运动更稳定
        max_angle=40,
        min_angle=-40,
        dt=0.01,
        turning_bias=10.0,      # 增大转向偏置，这是关键参数
        turning_offset=0.0      # 不需要额外的偏移
    )
...
def main():
    ...
    # # 创建运动模式字典
    # motion_patterns = create_motion_patterns()

    # # 选择一个具体的运动模式
    # params = motion_patterns["Forward"]  # 或者 "Turn_Left",
    ...
    # 设置仿真时间
    t_span = np.linspace(0, 30, 3000)  # 增加仿真时间以观察完整的圆周运动
