...
class CPGNetwork:
    ...
    def update(self, dt: float) -> np.ndarray:
        ...
        if self.params.is_circular:
            phase_adjustment = np.pi/3  # 调整相位差以优化圆周运动
        else:
        ...
        for i, osc in enumerate(self.oscillators):
            ...
            for j in range(self.n_oscillators):
                if self.coupling_matrix[i][j] != 0:
                    ...
                    if i < j:
                        phase_diff += self.params.phase_bias + phase_adjustment
                    else:
                        phase_diff -= self.params.phase_bias + phase_adjustment
            ...
            # 计算基础角度
            base_angle = self.params.amplitude * np.sin(phase)

            # 添加圆周运动效果
            if self.params.is_circular:
                # 指数衰减的转向效果
                turning_effect = self.params.circular_bias * \
                               np.exp(-i / (self.n_oscillators/2))
                # 线性衰减的偏移
                offset = self.params.circular_bias * 0.5 * \
                        (1 - i/(self.n_oscillators-1))

                # 添加额外的相位调整以改善圆周运动
                circular_phase = 2 * np.pi * i / self.n_oscillators
                circular_component = 10.0 * np.sin(circular_phase)

                joint_angles[i] = base_angle + turning_effect + offset + circular_component
            else:
                # 普通前进模式
                joint_angles[i] = base_angle
...
class SnakeRobot:
    ...
    def calculate_positions(self, joint_angles: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if self.params.is_circular:
            ...
            # 计算头部位置
            center_x = self.params.circular_radius  # 将圆心移到右侧
            center_y = 0.0

            # 头部在圆上的位置
            self.x = center_x + self.params.circular_radius * np.cos(self.theta)
            self.y = center_y + self.params.circular_radius * np.sin(self.theta)

            # 头部朝向为切线方向
            head_angle = self.theta + np.pi/2
        else:
            self.x += self.params.v_f * np.cos(self.theta) * self.params.dt
            self.y += self.params.v_f * np.sin(self.theta) * self.params.dt
            head_angle = self.theta
...
def create_circular_motion_params(radius: float = 2.0) -> CPGParams:
    """创建更适合圆周运动的参数"""
    params = CPGParams(
        n_segments=8,
        segment_length=0.1,
        frequency=2.0,          # 增加频率到2Hz
        amplitude=30.0,         # 增加振幅
        phase_bias=np.pi/2,     # 90度相位差
        coupling_weight=1.0,
        convergence_rate=1.0,
        v_f=3.0,               # 显著增加速度到0.8 m/s
        mu_t=0.3,
        mu_n=1.0,
        control_gain=15.0,
        max_angle=35.0,        # 增加最大角度
        min_angle=-35.0,
        dt=0.001
    )
...
def main():
    # 创建圆周运动参数
    params = create_circular_motion_params(radius=0.5)

    # 创建可视化器
    visualizer = SnakeVisualizer(params)

    # 设置仿真时间
    t_span = np.linspace(0, 120, 12000)  # 减少总时间，但保持相同的时间步长
